 ...
 */
package com.tolgaduran.moviedb.adapter;
...
public class TVMovieArrayAdapter extends CursorAdapter {
    ...
    */
    ...
    public void bindView(View view, final Context context, Cursor cursor) {
        ...
        final String url = cursor.getString(isTVBookmark ? Constants.TV_DETAILS_COL_POSTER_PATH : (isMovieBookmark ? Constants.MOV_DETAILS_COL_POSTER_PATH : (isMovie ? Constants.MOV_COL_POSTER_PATH : Constants.TV_COL_POSTER_PATH)));
        Picasso
                .with(context)
                .load(url)
        ...
        String date = cursor.getString(isTVBookmark ? Constants.TV_DETAILS_COL_FIRST_AIR_DATE : (isMovieBookmark ? Constants.MOV_DETAILS_COL_RELEASE_DATE : (isMovie ? Constants.MOV_COL_RELEASE_DATE : Constants.TV_COL_FIRST_AIR_DATE)));
        ...
        if (isMovie) {
            ...
            Cursor cursor1 = context.getContentResolver().query(
                    MovieContract.FavouritesTVs.buildMoviesUriWithTvId(cursor.getString(isTVBookmark ? Constants.TV_DETAILS_COL_TV_ID : Constants.TV_COL_ID)),
                    null, null, null, null);
            if (!(cursor1 == null || !(cursor1.moveToFirst()) || cursor1.getCount() == 0)) {
                fav = 1;
                cursor1.close();
        ...
        String rating = cursor.getString(isTVBookmark ? Constants.TV_DETAILS_COL_VOTE_AVERAGE : (isMovieBookmark ? Constants.MOV_DETAILS_COL_VOTE_AVERAGE : (isMovie ? Constants.MOV_COL_VOTE_AVERAGE : Constants.TV_COL_VOTE_AVERAGE)));
        ...
        String popularity = cursor.getString(isTVBookmark ? Constants.TV_DETAILS_COL_POPULARITY : (isMovieBookmark ? Constants.MOV_DETAILS_COL_POPULARITY : (isMovie ? Constants.MOV_COL_POPULARITY : Constants.TV_COL_POPULARITY)));
